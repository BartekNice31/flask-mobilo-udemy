...
path_database_config=r"C:\Users\Bartłomiej\Desktop\python\flask-mobilo-udemy-main\sql_alchemy\config_hotel.cfg"
...
class Notification(db.Model):
    ...
    guest_name=db.Column(db.String(30))
...
def init_app():
    ...
    active_admins = User.query.filter(User.is_active,User.is_admin).count()
    ...
    new_user=User(name=user_pass.user
                ,email='admin@wp.pl'
...
def notification(): 
    ...
    if request.method == 'GET': 
        ...
        new_notification=Notification(room_number=room_number
                                    ,guest_name=guest_name
                                    ,notification=notification_text
                                    ,priority=priority
                                    ,date_notification=datetime.utcnow().date())
...
def delete_notification(notification_id):  
    notification_to_delete=Notification.query.filter(Notification.id==notification_id).first()
    db.commit()  
...
def edit_notification(notification_id:int):
    notification_priorities = NotificationPriorities() 
    notification_priorities.load_priorities()  
    notif_obj=Notification.query.filter(Notification.id==notification_id).first()
    if request.method == 'GET':  
        notif_obj=Notification.query.filter(Notification.id==notification_id).first()
        ...
        if raise_priority: 
            priority = 'high' 
            flash('Rising priority from medium to high')  
        notif_obj.room_number=room_number
        notif_obj.guest_name=guest_name
        notif_obj.notification=notification_text
        notif_obj.priority=priority
        notif_obj.date_notification=datetime.utcnow().date()
        db.session.commit() 
...
def user_status_change(action,user_name):
    ...
    login=session['user'] 
    if action=='active':
        user=User.query.filter(User.name==user_name,User.name!=login).first()
        user.is_active=(user.is_active+1)%2
        db.session.commit()
        flash("CHANGE USER STATUS")
    elif action=='admin':
        user=User.query.filter(User.name==user_name,User.name!=login).first()
        user.is_active=(user.is_admin+1)%2
        db.session.commit()
...
def delete_user(user_name):
    ...
    if login==user_name:
        ...
        user_to_delete=User.query.filter(User.name==user_name)
